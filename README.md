# 🎤 Talks & Presentations
**Jishan Ahmed** · Weber State University

> Slides from invited talks, seminar presentations, and conference lightning talks — spanning AI, machine learning, statistics, and data science.

---

## 📁 Project Structure

```
Talks2026/
├── README.md
├── 2026/
│   ├── AITalk.pdf                ← From Text to Behavior (WSU, April 2026)
│   └── BSUTalk_NLP.pdf           ← Learning Market Signals (Ball State, March 2026)
├── 2025/
│   ├── JishanASALight.pdf        ← Imbalanced Data (SDSS Lightning Talk, April 2025)
│   └── SVDTalkSLC4_5_25.pdf      ← Hidden Structure in Big Data (SLC Math For All, April 2025)
└── 2024/
    └── ICAMathMonday.pdf         ← Cocktail Party Problem / ICA (Math Factor, Feb 2024)
```

---

## 🗓️ Talk Index

### 2026

| Date | Title | Venue | Location | Slides |
|------|-------|-------|----------|--------|
| Apr 22, 2026 | From Text to Behavior: Making Probabilistic AI Visible with Embodied Robots | Innovative Tech Showcase, Weber State University | Ogden, UT | [PDF](2026/AITalk.pdf) |
| Mar 24, 2026 | Learning Market Signals from Financial News Headlines Using Interpretable Machine Learning | Faculty & Graduate Research Colloquium, Dept. of Mathematical Sciences, Ball State University | Muncie, IN | [PDF](2026/BSUTalk_NLP.pdf) |

### 2025

| Date | Title | Venue | Location | Slides |
|------|-------|-------|----------|--------|
| Apr 30, 2025 | Assessing the Role of Data Complexity in Learning from Imbalanced Data | Symposium on Data Science & Statistics *(Two-Part Lightning Oral)* | Salt Lake City, UT | [PDF](2025/JishanASALight.pdf) |
| Apr 17, 2025 | Discovering Hidden Structure in Big Data with Simple Math | Salt Lake City Math For All Conference, University of Utah | Salt Lake City, UT | [PDF](2025/SVDTalkSLC4_5_25.pdf) |

### 2024

| Date | Title | Venue | Location | Slides |
|------|-------|-------|----------|--------|
| Sep 23, 2024 | The False Promise of ChatGPT: Uncovering the Truth Behind Large Language Models | Math Factor Seminar Series, Mathematics Club | Ogden, UT | *slides pending* |
| May 3, 2024 | Fundamentals of Large Language Models and Ethical Implications | AI Learning Community Workshop, Weber State University | Ogden, UT | *slides pending* |
| Feb 26, 2024 | The Cocktail Party Problem: Solving with Independent Component Analysis (ICA) | Math Factor Seminar Series, Mathematics Club | Ogden, UT | [PDF](2024/ICAMathMonday.pdf) |

---

## 🏷️ Topics

`Probabilistic AI` `Embodied Robotics` `NLP` `Interpretable ML` `Imbalanced Data` `SVD` `ICA` `Large Language Models` `Data Science`

---

## 👤 Author

**Jishan Ahmed** — [JishanAhmed2019](https://github.com/JishanAhmed2019)
Weber State University · Department of Mathematics

---

*See also: [WSUMath3410](https://github.com/JishanAhmed2019/WSUMath3410) · [math3410_fall25](https://github.com/JishanAhmed2019/math3410_fall25)*
